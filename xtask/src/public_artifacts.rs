...
fn render_wasm_types(root: &Path) -> Result<String, String> {
    ...
    output.push_str("export type VibelangExecutionFailurePhase = \"evaluate\" | \"initialize\" | \"bridge\" | \"dispatch\" | \"runtime\";\n\n");
    output.push_str("export interface VibelangMutationReceipt {\n  schema_version: number;\n  attempt_id: string;\n  runtime_epoch: string;\n  revision: string | null;\n  event_sequence: string;\n  request: unknown;\n  state: unknown;\n  previous_confirmed_revision: string | null;\n  timestamps: unknown;\n  diagnostics: unknown[];\n}\n\n");
...
fn wasm_value_interfaces(file: &syn::File) -> Result<String, String> {
    let names = [
        ...
        ("ExecutionFailure", "VibelangExecutionFailure"),
    ...
    for (rust_name, ts_name) in names {
        ...
        for field in &item.fields {
            ...
            if rust_name == "ExecutionResult" && name == "success" {
                output.push_str("  /** @deprecated Evaluation-only compatibility signal. Inspect receipt for runtime truth. */\n");
            }
    ...
    match ty {
        ...
        Type::Path(path) => {
            ...
            match name.as_str() {
                ...
                "ExecutionFailure" => "VibelangExecutionFailure".into(),
                "ExecutionFailurePhase" => "VibelangExecutionFailurePhase".into(),
                "MutationReceipt" => "VibelangMutationReceipt".into(),
                "Option" => {
...
fn wasm_method_return(_class: &str, method: &WasmMethod) -> String {
    match method.name.as_str() {
        ...
        "getReceipt" => "VibelangMutationReceipt".into(),
...
mod tests {
    ...
    #[test]
    fn wasm_receipt_carrier_types_do_not_repeat_legacy_success_as_terminal_truth() {
        let declarations = render_wasm_types(&root()).unwrap();
        assert!(declarations.contains(
            "/** @deprecated Evaluation-only compatibility signal. Inspect receipt for runtime truth. */\n  success: boolean;"
        ));
        assert!(declarations.contains("getReceipt(attempt_id: string): VibelangMutationReceipt;"));
    }
