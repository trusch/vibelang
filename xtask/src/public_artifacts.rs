...
pub fn generate(root: &Path, cli_help_path: &Path, check: bool) -> Result<(), String> {
    ...
    println!(
        "intentional exclusion: wasm-bindgen private InitOutput ABI fields and generated JS/wasm binaries are build outputs, not checked-in public artifacts; types/index.d.ts regenerates the annotated Rust export surface and retains only the stable module-init compatibility shim"
    );
    Ok(())
}
...
fn render_wasm_types(root: &Path) -> Result<String, String> {
    ...
    output.push_str("/* Raw wasm-bindgen JS/wasm and private InitOutput ABI fields are intentionally excluded. */\n\n");
