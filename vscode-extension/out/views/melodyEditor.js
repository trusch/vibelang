...
exports.MelodyEditor = void 0;
const vscode = require("vscode");
const melodyParser_1 = require("../utils/melodyParser");
...
class MelodyEditor {
    constructor(panel, store) {
        ...
        this._noteKeyAssignments = new Map();
        this._panel = panel;
        this._store = store;
        this._updateContent();
        ...
        this._disposables.push(store.onFullUpdate(() => this._refreshMelodyList()));
    ...
    static createOrShow(store, melodyName) {
        ...
        const panel = vscode.window.createWebviewPanel(MelodyEditor.viewType, 'Melody Editor', column, {
            enableScripts: true,
    ...
    async loadMelody(melodyName) {
        ...
        if (melody.notes_patterns && melody.notes_patterns.length > 0) {
            ...
            grid = (0, melodyParser_1.parseMultiLaneMelodyStrings)(melody.notes_patterns);
     ...
     */
    async _checkIfDynamicMelody(sourceLocation) {
    ...
    _eventsToGrid(events, loopBeats) {
        const notes = [];
        const beatsPerBar = 4;
        ...
        for (const event of events) {
            ...
            if (event.note) {
                ...
                const parsed = (0, melodyParser_1.parseNoteToMidi)(event.note);
                if (parsed !== null) {
                    midiNote = parsed;
                }
            }
    ...
    _autoAssignNotes() {
        ...
        baseNotes.forEach((midiNote, index) => {
            this._noteKeyAssignments.set(index, {
                midiNote,
                noteName: (0, melodyParser_1.midiToNoteName)(midiNote),
            });
        });
     ...
     */
    _autoAssignNotesFromScale(notes) {
        this._noteKeyAssignments.clear();
    ...
    async _refreshMelodyFromDocument() {
        if (!this._currentMelody)
            return;
        if (this._currentMelody.locallyModified)
        ...
        try {
            ...
            for (let lineIdx = startLineIndex; lineIdx < endLineIndex; lineIdx++) {
                ...
                if (match) {
                    ...
                    const currentMelodyString = (0, melodyParser_1.generateMelodyString)(this._currentMelody.grid, 4);
                    if (melodyString !== currentMelodyString) {
                        // Melody changed in code - update grid
                        this._currentMelody.grid = (0, melodyParser_1.parseMelodyString)(melodyString);
                        this._sendMelodyUpdate();
                    }
                    break;
                }
    ...
    async _handleMessage(message) {
        switch (message.command) {
            case 'ready':
                ...
                break;
            case 'loadMelody':
                this.loadMelody(message.melodyName);
                break;
            case 'updateGrid':
                if (this._currentMelody) {
                    this._currentMelody.grid = message.grid;
            ...
            case 'addNote': {
                if (this._currentMelody) {
                    const note = {
                        startBeat: message.startBeat,
                        duration: message.duration,
                        midiNote: message.midiNote,
                        velocity: message.velocity || 1.0,
                    };
            ...
            case 'resizeGrid':
                if (this._currentMelody) {
                    ...
                    const newGrid = (0, melodyParser_1.createEmptyMelodyGrid)(config);
            ...
            case 'previewNote':
                ...
                if (this._currentMelody?.voiceName) {
                    ...
                    const duration = message.duration || 0.25;
                    ...
                    try {
                        await this._store.runtime.noteOn(this._currentMelody.voiceName, midiNote, velocity);
                        setTimeout(async () => {
                            if (this._currentMelody?.voiceName) {
                                await this._store.runtime.noteOff(this._currentMelody.voiceName, midiNote);
                            }
                        }, duration * 1000);
                    }
                    catch {
            ...
            case 'recordNote': {
                ...
                if (this._currentMelody) {
                    const midiNote = message.midiNote;
                    const beat = message.beat;
                    const noteLength = message.noteLength || 0.25;
                    // Quantize to grid
            ...
            case 'autoAssignNotes':
                if (message.notes && Array.isArray(message.notes)) {
                    this._autoAssignNotesFromScale(message.notes);
                }
                else {
                    this._autoAssignNotes();
                }
            ...
            case 'clearMelody':
                ...
                break;
            case 'transpose': {
                const semitones = message.semitones;
                if (this._currentMelody) {
    ...
    async _updateMelodyViaApi() {
        ...
        const lanes = (0, melodyParser_1.generateMultiLaneMelodyStrings)(this._currentMelody.grid, stepsPerBar);
        try {
     ...
     */
    _calculateOptimalStepsPerBar(grid) {
            ...
            if (startFrac !== 0 || durFrac !== 0) {
                ...
                for (const div of [0.5, 0.25, 0.125, 0.0625, 0.03125]) {
                    if (Math.abs(note.startBeat % div) < 0.001 && Math.abs(note.duration % div) < 0.001) {
                        if (div < smallestDivision)
                            smallestDivision = div;
                        break;
                    }
                    if (div < smallestDivision)
                        smallestDivision = div;
    ...
    async _writeBackToFile() {
        ...
        if (laneCount <= 1) {
            ...
            notesCallsStr = sourceEmitters_1.vibe.member('Melody', 'notes', [sourceEmitters_1.vibe.string(lanes[0] || '')]);
        }
        else {
            // Multiple lanes - generate chained .notes() calls
            notesCallsStr = lanes.map(lane => sourceEmitters_1.vibe.member('Melody', 'notes', [sourceEmitters_1.vibe.string(lane)])).join('\n    ');
        }
        try {
            ...
            if (firstNotesLine !== -1) {
                ...
                edit.replace(document.uri, new vscode.Range(firstNotesLine, firstNotesStart, lastNotesLine, lastNotesEnd), notesCallsStr);
    ...
    _getHtmlContent() {
        ...
        const styles = `
            ${(0, sharedComponents_1.getEditorBaseStyles)()}
        ...
        `;
        const scripts = `
            ${(0, sharedComponents_1.getTransportUtilsScript)()}
            ${(0, sharedComponents_1.getNumpadUtilsScript)()}
            ${(0, sharedComponents_1.getRecordButtonScript)()}
            ${(0, sharedComponents_1.getToastScript)()}
...
<body>
    <div class="toolbar">
        ...
        ${(0, sharedComponents_1.renderTimingSlider)(-50)}

        <div class="toolbar-group">
            ${(0, sharedComponents_1.renderRecordButton)()}
        </div>
    </div>
    ...
    ${(0, sharedComponents_1.renderKeyAssignmentPanel)('notes', 'Numpad Recording', 'Press keys 1-9 to record notes', true)}
    ...
    ${(0, sharedComponents_1.renderCodePanel)('code', 'Generated Code', 'Edit the melody above to see code here...')}

    <!-- Toast notifications -->
    ${(0, sharedComponents_1.renderToastContainer)()}
...
exports.MelodyEditor = MelodyEditor;
MelodyEditor.viewType = 'vibelang.melodyEditor';
//# sourceMappingURL=melodyEditor.js.map