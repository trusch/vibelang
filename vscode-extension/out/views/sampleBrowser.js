 ...
 * - Preview playback
 * - Copyable code snippets for sample, voice, pattern, and slicing
...
const waveformProvider_1 = require("../utils/waveformProvider");
const tagStore_1 = require("../utils/tagStore");
...
class SampleBrowser {
    constructor(panel, store, context) {
        ...
        this._testVoiceActive = false; // Track if we have an active test voice
        this._activeTestNotes = new Set(); // Track ALL active test notes for proper cleanup
        this._currentTestSynthdef = null; // Track which synthdef/sample the test voice is for
        ...
        this._tagStore = new tagStore_1.TagStoreManager(context);
    ...
    static revive(panel, store, context) {
        SampleBrowser.currentPanel = new SampleBrowser(panel, store, context);
    }
    _loadStdlibData() {
        try {
    ...
    async _handleMessage(message) {
        switch (message.command) {
            ...
            case 'testSynthDefAtPitch':
                await this._testSynthDefAtPitch(message.synthdefName, message.midiNote, message.isOneShot, message.sustained, message.forceNewVoice);
                break;
            case 'testSampleAtPitch':
                await this._testSampleAtPitch(message.sampleId, message.midiNote, message.isOneShot, message.sustained, message.forceNewVoice);
                break;
            case 'releaseTestNote':
                await this._releaseTestNote(message.midiNote);
                break;
            case 'stopTestVoice':
                await this._stopTestSynthDef('');
                break;
            case 'enterChoppingMode':
                await this._enterChoppingMode(message.sampleId);
                break;
            case 'playSampleFull':
                await this._playSampleFull(message.sampleId);
                break;
            case 'stopSamplePlayback':
                await this._stopSamplePlayback();
                break;
            case 'previewSlice':
                await this._previewSlice(message.sampleId, message.startSeconds, message.endSeconds);
                break;
            case 'saveSlicesToFile':
                await this._saveSlicesToFile(message.sampleId, message.sliceCode);
                break;
            // Tag management commands
            case 'addTag':
                await this._tagStore.addTag(message.type, message.id, message.tag);
                break;
            case 'removeTag':
                await this._tagStore.removeTag(message.type, message.id, message.tag);
                break;
            case 'setTags':
                await this._tagStore.setUserTags(message.type, message.id, message.tags);
                break;
            case 'toggleTag':
                await this._tagStore.toggleTag(message.type, message.id, message.tag);
                break;
        }
    }
    async _loadSampleFromFile() {
        ...
        if (result && result.length > 0) {
            ...
            if (samplesFileExists) {
                ...
                const nameRegex = /let\s+(\w+)\s*=\s*sample/g;
                let nameMatch;
                while ((nameMatch = nameRegex.exec(existingContent)) !== null) {
                    existingNames.add(nameMatch[1]);
                }
            }
            // Generate sample definitions for each selected file
            const sampleStatements = result.map(uri => {
                ...
                return `let ${safeName} = ${sourceEmitters_1.vibe.free('sample', [sourceEmitters_1.vibe.string(safeName), sourceEmitters_1.vibe.string(uri.fsPath)])};`;
            });
            if (samplesFileExists) {
                // Append to existing samples file
                const existingContent = fs.readFileSync(samplesFilePath, 'utf-8');
                const newContent = existingContent.trimEnd() + '\n\n' + sampleStatements.join('\n') + '\n';
                ...
                const content = header + sampleStatements.join('\n') + '\n';
            ...
            vscode.window.showInformationMessage(`Added ${result.length} sample(s) to ${samplesFileName}`);
	...
	let ${id} = ${sourceEmitters_1.vibe.free('sample', [sourceEmitters_1.vibe.string(id), sourceEmitters_1.vibe.string(path)])};

	// Create a voice using the sample
	let ${id}_voice = ${sourceEmitters_1.vibe.free('voice', [sourceEmitters_1.vibe.string(`${id}_voice`)])}
	    ${sourceEmitters_1.vibe.member('Voice', 'on', [sourceEmitters_1.vibe.expr('SampleHandle', id)])};

	// Example pattern
	let ${id}_pattern = ${sourceEmitters_1.vibe.free('pattern', [sourceEmitters_1.vibe.string(`${id}_pattern`)])}
	    ${sourceEmitters_1.vibe.member('Pattern', 'on', [sourceEmitters_1.vibe.expr('Voice', `${id}_voice`)])}
	    ${sourceEmitters_1.vibe.member('Pattern', 'step', [sourceEmitters_1.vibe.string('x...x...x...x...')])}
    ...
    async _insertSynthDefCode(synthdefName) {
        ...
        const paramsCode = synthdef.params
            ...
            .map(p => `    ${sourceEmitters_1.vibe.member('Voice', 'param', [sourceEmitters_1.vibe.string(p.name), sourceEmitters_1.vibe.f64(p.default_value)])}`)
            .join('\n');
        const code = `let my_voice = ${sourceEmitters_1.vibe.free('voice', [sourceEmitters_1.vibe.string('my_voice')])}
    ${sourceEmitters_1.vibe.member('Voice', 'synth', [sourceEmitters_1.vibe.string(synthdef.name)])}${paramsCode ? '\n' + paramsCode : ''};`;
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            await editor.edit(edit => {
                edit.insert(editor.selection.active, code + '\n');
    ...
    async _testSynthDefAtPitch(synthdefName, midiNote, isOneShot, sustained, forceNewVoice) {
        const runtime = this._store.runtime;
        if (!runtime) {
            vscode.window.showErrorMessage('Not connected to VibeLang runtime');
            this._panel.webview.postMessage({ type: 'testStopped' });
    ...
    async _testSampleAtPitch(sampleId, midiNote, isOneShot, sustained, forceNewVoice) {
        const runtime = this._store.runtime;
        if (!runtime) {
            vscode.window.showErrorMessage('Not connected to VibeLang runtime');
            return;
    ...
    async _releaseTestNote(midiNote) {
        const runtime = this._store.runtime;
        if (!runtime || !this._testVoiceActive)
            return;
        try {
    ...
    async _enterChoppingMode(sampleId) {
        const sample = this._store.state?.samples.find(s => s.id === sampleId);
        if (!sample) {
        ...
        try {
            ...
            const waveformData = await (0, waveformProvider_1.getWaveformData)(sample.path, { targetPoints: 1000 });
    ...
    async _playSampleFull(sampleId) {
        const runtime = this._store.runtime;
        if (!runtime) {
            return;
        }
    ...
    async _previewSlice(sampleId, startSeconds, endSeconds) {
        const runtime = this._store.runtime;
        if (!runtime) {
            return;
        }
        try {
            ...
            const code = `
                let _preview_sample = ${sourceEmitters_1.vibe.free('sample', [sourceEmitters_1.vibe.string('_preview'), sourceEmitters_1.vibe.string(sample.path)])};
                let _preview_slice = _preview_sample${sourceEmitters_1.vibe.member('SampleHandle', 'slice', [sourceEmitters_1.vibe.f64(startSeconds), sourceEmitters_1.vibe.f64(endSeconds)])};
                let _preview_voice = ${sourceEmitters_1.vibe.free('voice', [sourceEmitters_1.vibe.string('_slice_preview')])}${sourceEmitters_1.vibe.member('Voice', 'on', [sourceEmitters_1.vibe.expr('SampleHandle', '_preview_slice')])};
                ${sourceEmitters_1.vibe.free('pattern', [sourceEmitters_1.vibe.string('_slice_preview_pattern')])}${sourceEmitters_1.vibe.member('Pattern', 'on', [sourceEmitters_1.vibe.expr('Voice', '_preview_voice')])}${sourceEmitters_1.vibe.member('Pattern', 'step', [sourceEmitters_1.vibe.string('x')])}${sourceEmitters_1.vibe.member('Pattern', 'start', [])};
            `;
    ...
    async _findSamplesFileForSample(sampleId) {
        ...
        for (const folder of workspaceFolders) {
            ...
            for (const file of files) {
                ...
                // Check if this file contains a sample constructor for our sampleId
                if (content.includes(`sample("${sampleId}"`)) {
                    return file.fsPath;
                }
            }
        }
    ...
    async _saveSlicesToFile(sampleId, sliceCode) {
...
<body>
    ...
    <script>
        ...
        function highlightCode(code) {
            ...
            var functions = ['voice', 'pattern', 'melody', 'group', 'synth', 'sample', 'start', 'stop', 'slice', 'warp_to_bpm', 'semitones', 'speed', 'pitch', 'attack', 'release', 'offset', 'length', 'loop_mode', 'amp', 'param'];
