...
class EffectRack {
    ...
    static createOrShow(store, groupPath) {
        ...
        const panel = vscode.window.createWebviewPanel(EffectRack.viewType, 'Effect Rack', column, {
            enableScripts: true,
    ...
    _sendStateUpdate() {
        ...
        if (state) {
            ...
            const effectSynthdefs = state.synthdefs.filter(s => s.name.startsWith('fx_') ||
    ...
    async _handleMessage(message) {
        switch (message.command) {
            ...
            case 'updateEffectParam':
                const effectId = message.effectId;
                const paramName = message.param;
            ...
            case 'insertEffectCode':
                await this._insertEffectCode(message.synthdefName, message.groupPath);
                break;
            case 'goToSource':
    ...
    async _insertEffectCode(synthdefName, groupPath) {
...
exports.EffectRack = EffectRack;
EffectRack.viewType = 'vibelang.effectRack';
//# sourceMappingURL=effectRack.js.map