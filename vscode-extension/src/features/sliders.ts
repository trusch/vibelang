...
interface Parameter {
    id: string; // unique id for the DOM
    name: string;
    value: number;
    line: number;
    startChar: number;
    ...
    max: number;
    step: number;
    isDb: boolean; // Track if the value is inside a db(...) call
    contextName?: string;
}

export class ParameterSlidersPanel {
    public static currentPanel: ParameterSlidersPanel | undefined;
    public static readonly viewType = 'vibelangSliders';
    ...
    private _isWebviewUpdating = false; // Flag to prevent infinite loops
    private _lastParams: Parameter[] = []; // To diff against
    ...
    public static createOrShow(extensionUri: vscode.Uri) {
        ...
        const panel = vscode.window.createWebviewPanel(
            ParameterSlidersPanel.viewType,
            'Vibelang Parameters',
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri) {
        ...
        vscode.workspace.onDidChangeTextDocument(e => {
            if (this._currentDocument && e.document.uri.toString() === this._currentDocument.uri.toString()) {
                if (this._isWebviewUpdating) return;
    ...
    private _handleSelectionChange(line: number, character: number) {
    ...
    private _areParamsStructurallyEqual(oldParams: Parameter[], newParams: Parameter[]): boolean {
    ...
    private _updateParamValues(params: Parameter[]) {
        this._panel.webview.postMessage({
    ...
    private _parseParameters(doc: vscode.TextDocument): Parameter[] {
    ...
    private _determineContextName(line: string, valueIndex: number, value: number): string {
    ...
    private _determineRange(name: string, value: number, lineText: string, isDb: boolean) {
        let min = 0;
        let max = 1;
        let step = 0.01;
    ...
    private _isInsideString(line: string, index: number): boolean {
        let inSingle = false;
        let inDouble = false;
    ...
    private _findParentContext(lines: string[], lineIndex: number, charIndex: number): string | undefined {
    ...
    private _updateParameterInDocument(line: number, startChar: number, _ignoredEndChar: number, value: number) {
    ...
    private _triggerAutoSave(doc: vscode.TextDocument) {
    ...
    private _getHtmlForWebview(params: Parameter[]) {
