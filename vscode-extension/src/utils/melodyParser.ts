 ...
 */
export function detectChordFromMidiNotes(midiNotes: number[]): { root: number; quality: string } | null {
    ...
    const chordTypes: Array<{ name: string; intervals: number[] }> = [
...
// =============================================================================
// Polyphonic Melody Support - Lane Splitting
// =============================================================================

/**
 * Extended note interface with lane index for polyphonic support.
 */
export interface MelodyNoteWithLane extends MelodyNote {
    laneIndex: number;
}
 ...
 */
export function splitIntoLanes(notes: MelodyNote[]): MelodyNoteWithLane[][] {
    if (notes.length === 0) return [[]];

    // Sort by start beat, then by pitch (for determinism)
    const sorted = [...notes].sort((a, b) =>
        a.startBeat - b.startBeat || a.midiNote - b.midiNote
    );

    const lanes: MelodyNoteWithLane[][] = [];
 ...
 */
export function generateMultiLaneMelodyStrings(grid: MelodyGrid, stepsPerBar: number = 4): string[] {
 ...
 */
export function parseMultiLaneMelodyStrings(lanes: string[], config?: Partial<MelodyConfig>): MelodyGrid {
    if (lanes.length === 0) {
 ...
 */
export function countLanes(notes: MelodyNote[]): number {
