...
import { ConnectionStatus } from '../api/runtimeManager';
import { TransportState } from '../api/types';
...
export class TransportBar {
    ...
    // Status bar items (right to left priority)
    private _connectionItem: vscode.StatusBarItem;
    private _playStopItem: vscode.StatusBarItem;
    private _bpmItem: vscode.StatusBarItem;
    private _beatItem: vscode.StatusBarItem;
    private _timeItem: vscode.StatusBarItem;
    ...
    constructor(store: StateStore) {
        ...
        // Play/Stop button
        this._playStopItem = vscode.window.createStatusBarItem(
            vscode.StatusBarAlignment.Left,
            99
        );
        ...
        // BPM display/control
        this._bpmItem = vscode.window.createStatusBarItem(
            vscode.StatusBarAlignment.Left,
            98
        );
        ...
        this._beatItem = vscode.window.createStatusBarItem(
            vscode.StatusBarAlignment.Left,
            97
        );
        ...
        this._timeItem = vscode.window.createStatusBarItem(
            vscode.StatusBarAlignment.Left,
            96
        );
        ...
        this._disposables.push(
            this._store.onStatusChange((status) => this.updateConnection(status))
        );
        this._disposables.push(
            this._store.onTransportUpdate((transport) => this.updateTransport(transport))
        );
    ...
    private updateConnection(status: ConnectionStatus) {
        switch (status) {
            case 'connected':
    ...
    private updateTransport(transport: TransportState) {
        // Play/Stop button
        if (transport.running) {
            this._playStopItem.text = '$(debug-pause)';
            this._playStopItem.backgroundColor = new vscode.ThemeColor(
        ...
        const { bar, beatInBar, subdivision } = this.beatsToBarTime(
            displayBeat,
     ...
     */
    private beatsToBarTime(
        totalBeats: number,
        beatsPerBar: number
    ): { bar: number; beatInBar: number; subdivision: number } {
     ...
     */
    static registerCommands(context: vscode.ExtensionContext, store: StateStore) {
        // Toggle connection
        context.subscriptions.push(
            vscode.commands.registerCommand('vibelang.toggleConnection', async () => {
                if (store.status === 'connected') {
                    ...
                    if (!connected) {
                        const action = await vscode.window.showErrorMessage(
                            'Could not connect to VibeLang runtime',
                            'Configure',
                            'Retry'
                        );
                        if (action === 'Configure') {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('vibelang.toggleTransport', async () => {
        ...
        // Start transport
        context.subscriptions.push(
            vscode.commands.registerCommand('vibelang.startTransport', async () => {
                await store.runtime.startTransport();
            })
        );
        ...
        // Stop transport
        context.subscriptions.push(
            vscode.commands.registerCommand('vibelang.stopTransport', async () => {
                await store.runtime.stopTransport();
            })
        );
        ...
        // Set BPM
        context.subscriptions.push(
            vscode.commands.registerCommand('vibelang.setBpm', async () => {
        ...
        // Seek to beat
        context.subscriptions.push(
            vscode.commands.registerCommand('vibelang.seekBeat', async () => {
        ...
        context.subscriptions.push(
            vscode.commands.registerCommand('vibelang.configureConnection', async () => {
