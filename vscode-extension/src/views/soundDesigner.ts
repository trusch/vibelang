...
import { WEBVIEW_VIBE_EMITTER_RUNTIME } from '../utils/sourceEmitters';
...
export class SoundDesignerPanel {
    public static currentPanel: SoundDesignerPanel | undefined;
    public static readonly viewType = 'vibelang.soundDesigner';

    private readonly _panel: vscode.WebviewPanel;
    private readonly _extensionPath: string;
    private readonly _store: StateStore;
    private _disposables: vscode.Disposable[] = [];
    private _ugenCategories: UGenCategory[] = [];

    private constructor(panel: vscode.WebviewPanel, extensionPath: string, store: StateStore) {
        this._panel = panel;
        ...
        this._disposables.push(
            store.onStatusChange(() => this._sendConnectionStatus())
        );
    ...
    public static createOrShow(extensionPath: string, store: StateStore) {
    ...
    public static revive(panel: vscode.WebviewPanel, extensionPath: string, store: StateStore) {
        SoundDesignerPanel.currentPanel = new SoundDesignerPanel(panel, extensionPath, store);
    }

    private _loadUGenManifests() {
    ...
    private _addBuiltInNodes() {
        const utilityNodes: UGenManifest[] = [
            {
                name: 'Add',
    ...
    private async _handleMessage(message: { command: string; [key: string]: unknown }) {
        switch (message.command) {
            case 'generateCode':
                ...
                break;
            case 'savePreset':
                const presetData = message.data as string;
...
class SoundDesigner {
    ...
    createNode(name, category, color, x, y, config = {}) {
        ...
        const node = {
            ...
            rate: ugenData && ugenData.rates.includes('ar')
                ? 'ar'
                : ugenData && ugenData.rates.includes('kr') ? 'kr' : 'ir',
            params: {},
    ...
    generateCode() {
        ...
        finalEssentialParams.forEach(p => {
            builderCode += '    ' + vibe.member('SynthDefBuilderHandle', 'param', [vibe.string(p.name), vibe.f64(p.value)]) + '\\n';
        });

        // Add user-defined parameters (ensure float format for Rhai)
        userParams.forEach(p => {
            const val = parseFloat(p.value);
            builderCode += '    ' + vibe.member('SynthDefBuilderHandle', 'param', [vibe.string(p.name), vibe.f64(val)]) + '\\n';
        });
        ...
        envNodes.forEach((node, i) => {
            ...
            let envelopeCode = vibe.free('envelope', []);
            if (env.type === 'adsr') {
                envelopeCode += vibe.member('EnvelopeBuilder', 'adsr', [vibe.f64(env.a), vibe.f64(env.d), vibe.f64(env.s), vibe.f64(env.r)]);
            } else if (env.type === 'asr') {
                envelopeCode += vibe.member('EnvelopeBuilder', 'asr', [vibe.f64(env.a), vibe.f64(env.s), vibe.f64(env.r)]);
            } else {
                envelopeCode += vibe.member('EnvelopeBuilder', 'perc', [vibe.f64(env.a), vibe.f64(env.r)]);
            }
            envelopeCode += vibe.member('EnvelopeBuilder', 'gate', [vibe.expr('NodeRef', 'gate')]);
            envelopeCode += vibe.member('EnvelopeBuilder', 'cleanup_on_finish', []);
            envelopeCode += vibe.member('EnvelopeBuilder', 'build', []);
            bodyCode += '    let ' + varName + ' = ' + envelopeCode + ';\\n\\n';
        });
        ...
        lfoNodes.forEach((node, i) => {
            ...
            let oscillator = vibe.ugen('sin_osc_kr', [vibe.f64(lfo.rate)]);
            if (lfo.wave === 'saw') oscillator = vibe.ugen('lf_saw_kr', [vibe.f64(lfo.rate)]);
            else if (lfo.wave === 'tri') oscillator = vibe.ugen('lf_tri_kr', [vibe.f64(lfo.rate)]);
            else if (lfo.wave === 'square') oscillator = vibe.ugen('lf_pulse_kr', [vibe.f64(lfo.rate)]);

            const scaled = vibe.operator('NodeRef', '*', 'f64', oscillator, vibe.f64(lfo.depth));
            bodyCode += '    let ' + varName + ' = ' + scaled.source + ';\\n\\n';
        });

        // Signal chain
        bodyCode += this.generateSignalChain();

        const body = '|'+ paramNames.join(', ') + '| {\\n' + bodyCode + '}';
        builderCode += '    ' + vibe.member('SynthDefBuilderHandle', 'body', [vibe.fn(body)]);
        return '// VibeLang Synthesizer\\n// Generated by Sound Designer\\n\\n'
            + vibe.free('define_synthdef', [vibe.string(name), vibe.fn('|builder| {\\n    ' + builderCode + '\\n}')])
            + ';\\n';
    }
    ...
    generateSignalChain() {
        ...
        const process = (nodeId) => {
            if (visited.has(nodeId)) return vibe.expr('NodeRef', getVar(nodeId));
            visited.add(nodeId);

            const node = this.nodes.find(n => n.id === nodeId);
            if (!node) return vibe.f64(0);

            if (node.type === 'param') return vibe.expr('NodeRef', node.name);

            if (node.type === 'output') {
                const conn = this.connections.find(c => c.to.node === nodeId);
                return conn ? process(conn.from.node) : vibe.f64(0);
            }
            ...
            if (node.inputs) {
                node.inputs.forEach((inp, i) => {
                    ...
                    if (conn) {
                        ...
                        args.push(vibe.f64(node.params[inp.name] ?? inp.default));
                    }
                });
            }
            ...
            if (node.name === 'Add') {
                ...
                const nonZeroArgs = args.filter(a => a.source !== '0.0');
                if (nonZeroArgs.length === 0) expr = vibe.f64(0);
                else expr = nonZeroArgs.slice(1).reduce((left, right) => vibe.add(left, right), nonZeroArgs[0]);
            } else if (node.name === 'Mul') {
                // Filter out default ones for cleaner code
                const nonOneArgs = args.filter(a => a.source !== '1.0');
                if (nonOneArgs.length === 0) expr = vibe.f64(1);
                else expr = nonOneArgs.slice(1).reduce((left, right) => vibe.multiply(left, right), nonOneArgs[0]);
            } else if (node.name === 'Scale') {
                expr = vibe.add(vibe.multiply(args[0], args[1]), args[2]);
            } else if (node.name === 'Mix') {
                expr = vibe.add(
                    vibe.multiply(args[0], vibe.subtract(vibe.f64(1), args[2])),
                    vibe.multiply(args[1], args[2]),
                );
            } else if (node.name === 'Const') {
                expr = args[0];
            } else if (node.envelope) {
                const idx = this.nodes.filter(n => n.envelope).indexOf(node);
                expr = vibe.expr('NodeRef', idx === 0 ? 'env' : 'env' + idx);
            } else if (node.lfo) {
                const idx = this.nodes.filter(n => n.lfo).indexOf(node);
                expr = vibe.expr('NodeRef', idx === 0 ? 'lfo' : 'lfo' + idx);
            } else {
                // Regular UGen
                expr = vibe.ugen(vibe.ugenName(node.name, node.rate), args);
            }

            code += '    let ' + varName + ' = ' + expr.source + ';\\n';
            return vibe.expr(expr.type, varName);
        };
        ...
        if (outputConn) {
            ...
            code += '    ' + vibe.f64(0).source + ' // Connect to Output\\n';
        }
