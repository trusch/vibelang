...
export class ArrangementTimeline {
    public static currentPanel: ArrangementTimeline | undefined;
    public static readonly viewType = 'vibelang.arrangement';
    ...
    private _getAvailableAutomationTargets(state: SessionState): AutomationTarget[] {
    ...
    private _sendTransportUpdate(transport: TransportState) {
        this._panel.webview.postMessage({
    ...
    private _buildTimelineData(state: SessionState) {
        ...
        const playingPatterns = state.patterns.filter(p =>
            p.is_looping || p.status?.state === 'playing' || p.status?.state === 'queued'
        );
        const playingMelodies = state.melodies.filter(m =>
            m.is_looping || m.status?.state === 'playing' || m.status?.state === 'queued'
        );
        ...
        const getItemLoopBeats = (name: string, type: string): number => {
            if (type === 'pattern') return patternsByName.get(name)?.loop_beats || 4;
            if (type === 'melody') return melodiesByName.get(name)?.loop_beats || 4;
            if (type === 'sequence') return sequencesByName.get(name)?.loop_beats || 4;
            return 4;
        };

        // Helper to get source location
        const getSourceLocation = (name: string, type: string) => {
            if (type === 'pattern') return patternsByName.get(name)?.source_location;
            if (type === 'melody') return melodiesByName.get(name)?.source_location;
            if (type === 'sequence') return sequencesByName.get(name)?.source_location;
            return undefined;
        };

        // Find which group each active sequence belongs to
        const findGroupForSequence = (seq: Sequence): string | null => {
            for (const clip of seq.clips) {
                if (clip.type === 'pattern') {
                    const p = patternsByName.get(clip.name);
                    if (p?.group_path) return p.group_path;
                }
                if (clip.type === 'melody') {
                    const m = melodiesByName.get(clip.name);
                    if (m?.group_path) return m.group_path;
                }
        ...
        const groupActiveSequences = new Map<string, Sequence[]>();
        for (const active of state.live.active_sequences) {
            const seq = sequencesByName.get(active.name);
            if (!seq) continue;
        ...
        for (const group of state.groups) {
            ...
            if (activeSeqs.length === 0) continue;
            ...
            for (const seq of activeSeqs) {
                ...
                const collectUniqueNames = (clips: SequenceClip[]): Set<string> => {
                ...
                // CLIP LANES (depth 2) - flatten all patterns/melodies from clips
                // When a clip references a sub-sequence, recurse into it to get the actual patterns/melodies
                // Group by pattern/melody name so multiple occurrences appear on the same lane

                // Collect all pattern/melody occurrences with their absolute positions
                ...
                const collectClips = (
                    ...
                    parentStart: number,
                    parentEnd: number,
                    parentLoopBeats: number
                ): ClipOccurrence[] => {
                    const result: ClipOccurrence[] = [];

                    for (const clip of clips) {
                        const clipStart = (clip.start_beat || 0) + parentStart;
                        const clipEnd = Math.min(
                            (clip.end_beat ?? parentLoopBeats) + parentStart,
                            parentEnd
                        );
                ...
                const allClipOccurrences: ClipOccurrence[] = [];
                ...
                for (let loop = 0; loop < numSeqLoops; loop++) {
                    ...
                    if (loopStart >= maxLoopBeats) break;
                ...
                // Group by name
                const clipsByName = new Map<string, ClipOccurrence[]>();
                ...
                for (const [clipName, occurrences] of clipsByName) {
                    ...
                    const timelineClips: TimelineClip[] = occurrences.map((occ, i) => ({
                        id: `${laneId}:${i}`,
                        name: clipName,
                        type: clipType,
        ...
        if (!hasClipLanes && (playingPatterns.length > 0 || playingMelodies.length > 0)) {
            ...
            const groupPlayingItems = new Map<string, { patterns: Pattern[]; melodies: Melody[] }>();
            for (const p of playingPatterns) {
            ...
            for (const group of state.groups) {
                ...
                for (const pattern of playingItems.patterns) {
                    ...
                    for (let i = 0; i < numReps; i++) {
                        ...
                        if (blockStart >= maxLoopBeats) break;
            ...
            for (const group of state.groups) {
                ...
                if (groupSeqs.length === 0) continue;
    ...
    private _getClipColor(type: string): string {
        switch (type) {
            case 'pattern':
                return '#9bbb59';
            case 'melody':
    ...
    private _getClipSourceLocation(clip: SequenceClip, state: SessionState) {
        switch (clip.type) {
            case 'pattern':
    ...
    private async _handleMessage(message: { command: string; [key: string]: unknown }) {
        switch (message.command) {
            ...
            case 'stopTransport':
                ...
                break;
            case 'seek':
                await this._store.runtime.seekTransport(message.beat as number);
                break;
            case 'startPattern':
                await this._store.runtime.startPattern(message.name as string);
                break;
            case 'stopPattern':
                await this._store.runtime.stopPattern(message.name as string);
                break;
            case 'startMelody':
                await this._store.runtime.startMelody(message.name as string);
                break;
            case 'stopMelody':
                await this._store.runtime.stopMelody(message.name as string);
                break;
            case 'startSequence':
                await this._store.runtime.startSequence(message.name as string);
                break;
            case 'stopSequence':
                await this._store.runtime.stopSequence(message.name as string);
                break;
            case 'muteTrack':
                const track = message.trackId as string;
            ...
            case 'selectTrack':
                this._handleTrackSelection(message.trackId as string, message.trackType as string);
                break;
            ...
            case 'addAutomationLane':
                this._addAutomationLane(message.target as AutomationTarget);
                break;

            case 'removeAutomationLane':
                this._removeAutomationLane(message.laneId as string);
                break;

            case 'toggleAutomationLane':
                this._toggleAutomationLaneVisibility(message.laneId as string);
                break;

            case 'addAutomationPoint':
                this._addAutomationPoint(
                    message.laneId as string,
                    message.beat as number,
                    message.value as number
                );
                break;

            case 'updateAutomationPoint':
                this._updateAutomationPoint(
                    message.laneId as string,
                    message.pointId as string,
                    message.beat as number,
                    message.value as number,
                    message.curveType as CurveType | undefined
                );
                break;

            case 'removeAutomationPoint':
                this._removeAutomationPoint(
                    message.laneId as string,
                    message.pointId as string
                );
                break;
            ...
            case 'setAutomationCurveType':
                this._setPointCurveType(
                    message.laneId as string,
                    message.pointId as string,
                    message.curveType as CurveType
                );
                break;

            case 'generateAutomationCode':
                this._generateAndInsertAutomationCode(message.laneId as string);
                break;

            case 'clearAutomationLane':
                this._clearAutomationLane(message.laneId as string);
                break;
    ...
    private _addAutomationLane(target: AutomationTarget) {
        // Check if lane already exists
        const existing = this._automationLanes.find(
            lane => lane.target.type === target.type &&
        ...
        const lane = createAutomationLane(target, minValue, maxValue);
        this._automationLanes.push(lane);
        this._sendAutomationUpdate();
    }

    private _removeAutomationLane(laneId: string) {
        this._automationLanes = this._automationLanes.filter(l => l.id !== laneId);
        this._sendAutomationUpdate();
    }

    private _toggleAutomationLaneVisibility(laneId: string) {
    ...
    private _addAutomationPoint(laneId: string, beat: number, value: number) {
        const lane = this._automationLanes.find(l => l.id === laneId);
        if (lane) {
            const point = createAutomationPoint(beat, value, 'smooth');
    ...
    private _updateAutomationPoint(
        laneId: string,
        pointId: string,
        beat: number,
        value: number,
    ...
    private _removeAutomationPoint(laneId: string, pointId: string) {
        const lane = this._automationLanes.find(l => l.id === laneId);
        if (lane) {
    ...
    private _setPointCurveType(laneId: string, pointId: string, curveType: CurveType) {
        const lane = this._automationLanes.find(l => l.id === laneId);
        if (lane) {
            const point = lane.points.find(p => p.id === pointId);
            if (point) {
    ...
    private _clearAutomationLane(laneId: string) {
        const lane = this._automationLanes.find(l => l.id === laneId);
        if (lane) {
            lane.points = [];
            this._sendAutomationUpdate();
    ...
    private async _generateAndInsertAutomationCode(laneId: string) {
        const lane = this._automationLanes.find(l => l.id === laneId);
    ...
    private _handleTrackSelection(trackId: string, trackType: string) {
...
interface TimelineClip {
    ...
    stepPattern?: string;
    // Melody visualization data
    melodyEvents?: Array<{ beat: number; midiNote: number; duration: number; velocity: number }>;
}
...
interface TimelineTrack {
    ...
    sourceLocation?: { file?: string; line?: number; column?: number };
    // Hierarchy support
    depth: number;           // 0 = top level (groups), 1 = sequences
    parentId?: string;       // ID of parent track
    isExpanded?: boolean;    // Whether children are visible
    hasChildren?: boolean;   // Whether this track has child tracks
    childCount?: number;     // Number of children (for display)
    // Mixer controls (for groups)
    groupPath?: string;      // The group path for API calls
    amp?: number;            // Volume (0-1)
    pan?: number;            // Pan (-1 to 1)
    effectsCount?: number;   // Number of effects on this group
    // Pattern data (for pattern clips)
    stepPattern?: string;    // Original step pattern string for visualization
    // Melody data (for melody clips)
    melodyEvents?: Array<{ beat: number; midiNote: number; duration: number; velocity: number }>;
}
