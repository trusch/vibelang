...
export class InspectorPanel {
    ...
    public static readonly viewType = 'vibelang.inspector';
    ...
    private constructor(panel: vscode.WebviewPanel, store: StateStore) {
        ...
        this._disposables.push(
            store.onSelectionChange(() => this._updateContent())
        );
    ...
    private async _handleMessage(message: { command: string; [key: string]: unknown }) {
        switch (message.command) {
            ...
            case 'interactionEnd':
                ...
                break;
            case 'setParam':
                await this._setParam(
                    message.entityType as string,
                    message.entityId as string,
                    message.param as string,
                    message.value as number
                );
                break;
            case 'mute':
                await this._toggleMute(message.entityType as string, message.entityId as string);
                break;
            case 'solo':
                await this._toggleSolo(message.entityType as string, message.entityId as string);
                break;
            case 'start':
                await this._start(message.entityType as string, message.entityId as string);
                break;
            case 'stop':
                await this._stop(message.entityType as string, message.entityId as string);
                break;
            case 'goToSource':
                const entity = this._store.getSelectedEntity();
    ...
    private async _setParam(entityType: string, entityId: string, param: string, value: number) {
        switch (entityType) {
            case 'group':
    ...
    private async _toggleMute(entityType: string, entityId: string) {
        const entity = this._store.getSelectedEntity();
        if (!entity || !('muted' in entity)) return;
    ...
    private async _toggleSolo(entityType: string, entityId: string) {
        const entity = this._store.getSelectedEntity();
        if (!entity || !('soloed' in entity)) return;

        if (entityType === 'group') {
            if ((entity as Group).soloed) {
    ...
    private async _start(entityType: string, entityId: string) {
        switch (entityType) {
            case 'pattern':
    ...
    private async _stop(entityType: string, entityId: string) {
        switch (entityType) {
            case 'pattern':
    ...
    private _getHtmlContent(
        ...
        entity: Group | Voice | Pattern | Melody | Sequence | Effect | undefined
    ): string {
        if (!selection || !entity) {
        ...
        switch (selection.type) {
            case 'group':
                content = this._renderGroup(entity as Group);
                break;
            case 'voice':
                content = this._renderVoice(entity as Voice);
                break;
            case 'pattern':
                content = this._renderPattern(entity as Pattern);
                break;
            case 'melody':
                content = this._renderMelody(entity as Melody);
                break;
            case 'sequence':
                content = this._renderSequence(entity as Sequence);
                break;
            case 'effect':
                content = this._renderEffect(entity as Effect);
                break;
        }
    ...
    private _renderGroup(group: Group): string {
    ...
    private _renderVoice(voice: Voice): string {
    ...
    private _renderPattern(pattern: Pattern): string {
    ...
    private _renderMelody(melody: Melody): string {
    ...
    private _renderSequence(sequence: Sequence): string {
    ...
    private _renderEffect(effect: Effect): string {
    ...
    private _renderParams(params: Record<string, number>, entityType: string, entityId: string): string {
        ...
        const normalizedParams = new Map<string, { originalKey: string; value: number }>();
    ...
    private _renderMiniTimeline(beats: number[], loopLength: number): string {
        const width = 200;
