...
interface PatternLane {
    ...
    isPlaying: boolean;
    color: string;
    /** True when the lane has been modified via pattern editor but not saved to source file */
    locallyModified?: boolean;
}

export class PatternEditor {
    public static currentPanel: PatternEditor | undefined;
    public static readonly viewType = 'vibelang.patternEditor';

    private readonly _panel: vscode.WebviewPanel;
    private readonly _store: StateStore;
    private _disposables: vscode.Disposable[] = [];
    private _lanes: PatternLane[] = [];
    private _selectedGroup: string | null = null;
    private _pendingPatternName: string | null = null;
    private _webviewReady: boolean = false;

    // Recording feature: rolling buffer and voice key assignments
    private static readonly MAX_BUFFER_BARS = 256;
    private _eventBuffer: Map<string, Array<{ beat: number; velocity: number }>> = new Map();
    private _voiceKeyAssignments: Map<number, { voiceName: string; patternName: string }> = new Map();
    ...
    private constructor(panel: vscode.WebviewPanel, store: StateStore) {
        ...
        this._disposables.push(
            store.onFullUpdate(() => this._refreshFromState())
        );
    ...
    public static createOrShow(store: StateStore, patternName?: string) {
    ...
    public loadPattern(patternName: string) {
        this._loadPatternByName(patternName);
    }

    private _loadPatternByName(patternName: string) {
    ...
    private _loadGroupPatterns(groupPath: string) {
        const state = this._store.state;
        if (!state) return;
     ...
     */
    private _sendVoiceAssignmentsUpdate() {
        const assignments: Array<{ keyIndex: number; voiceName: string; patternName: string }> = [];
    ...
    private _patternToLane(pattern: Pattern, index: number): PatternLane {
        ...
        if (patternString) {
            ...
            // Default empty grid
            grid = createEmptyGrid({ stepsPerBar: 16, numBars: 1, beatsPerBar: 4 });
        }
     ...
     */
    private async _refreshLanesFromDocuments() {
        let lanesChanged = false;
        ...
        for (let i = 0; i < this._lanes.length; i++) {
            ...
            if (!lane.sourceLocation?.file || !lane.sourceLocation?.line) continue;
            ...
            try {
                ...
                if (!document) continue; // Only update from open documents for responsiveness

                const startLineIndex = lane.sourceLocation.line - 1;
                if (startLineIndex < 0 || startLineIndex >= document.lineCount) continue;
                ...
                for (let lineIdx = startLineIndex; lineIdx < endLineIndex; lineIdx++) {
                    ...
                    if (match) {
                        ...
                        const currentGridString = generatePatternString(lane.grid);

                        if (patternString !== currentGridString) {
                            // Pattern changed in code - update grid
                            lane.grid = parsePatternString(patternString);
                            lanesChanged = true;
                        }
    ...
    private async _handleMessage(message: { command: string; [key: string]: unknown }) {
        switch (message.command) {
            ...
            case 'selectGroup':
                ...
                this._loadGroupPatterns(message.groupPath as string);
                break;

            case 'addPattern':
                const patternName = message.patternName as string;
                await this._addPatternToLanes(patternName);
                break;

            case 'addVoiceLane': {
                const voiceName = message.voiceName as string;
                const voiceConfig = message.config as PatternConfig | undefined;
                await this._addVoiceLane(voiceName, voiceConfig);
                break;
            }

            case 'removePattern':
                const removeIndex = message.laneIndex as number;
                this._lanes.splice(removeIndex, 1);
                this._sendLanesUpdate();
                break;

            case 'updateStep':
                const laneIndex = message.laneIndex as number;
                const stepIndex = message.stepIndex as number;
                const velocity = message.velocity as number;
                const accent = message.accent as boolean;
            ...
            case 'updateLane':
                const updateLaneIndex = message.laneIndex as number;
                const newGrid = message.grid as PatternGrid;
            ...
            case 'resizeGrid':
                const config = message.config as PatternConfig;
                await this._resizeAllLanes(config);
                break;

            case 'togglePlayback':
                const playLaneIndex = message.laneIndex as number;
            ...
            case 'goToSource':
                const sourceLaneIndex = message.laneIndex as number;
            ...
            case 'applyEuclidean':
                const eucLaneIndex = message.laneIndex as number;
                const hits = message.hits as number;
            ...
            case 'clearLane':
                const clearLaneIndex = message.laneIndex as number;
            ...
            case 'assignVoiceToKey': {
                ...
                const assignVoiceName = message.voiceName as string;
    ...
    private async _addPatternToLanes(patternName: string) {
        const state = this._store.state;
        if (!state) return;
     ...
     */
    private async _addVoiceLane(voiceName: string, config?: PatternConfig) {
        // Check if already in lanes
        ...
        const newLane: PatternLane = {
            patternName: `${voiceName}_pattern`,
            voiceName: voiceName,
            groupPath: this._selectedGroup || '',
            grid: createEmptyGrid({ stepsPerBar, numBars, beatsPerBar: 4 }),
     ...
     */
    private async _updatePatternViaApi(laneIndex: number) {
        const lane = this._lanes[laneIndex];
        if (!lane) return;
     ...
     * Handles multi-line pattern definitions by searching forward from the source location.
     */
    private async _updateLaneCode(laneIndex: number) {
        const lane = this._lanes[laneIndex];
        if (!lane?.sourceLocation?.file || !lane?.sourceLocation?.line) {
            return false;
        }

        const newPatternString = generatePatternString(lane.grid);
     ...
     */
    private async _writeBackAllLanesToFile() {
        let successCount = 0;
        let failCount = 0;
     ...
     */
    private _generateCodeFromVisualGrid(): string {
    ...
    private async _resizeAllLanes(config: PatternConfig) {
        for (let i = 0; i < this._lanes.length; i++) {
            const lane = this._lanes[i];
    ...
    private async _applyEuclidean(laneIndex: number, hits: number) {
        const lane = this._lanes[laneIndex];
        if (!lane) return;
    ...
    private _generateEuclidean(hits: number, steps: number): Array<{ velocity: number; accent: boolean }> {
        if (steps === 0) return [];
        if (hits >= steps) {
        ...
        const pattern: Array<{ velocity: number; accent: boolean }> = [];
        let bucket = 0;
     ...
     */
    private _reconstructPatternFromEvents(
        events: Array<{ beat: number; params?: Record<string, number> }>,
        loopBeats: number
    ): PatternGrid {
        ...
        const grid = createEmptyGrid({ stepsPerBar, numBars, beatsPerBar: 4 });
     ...
     */
    private _getBufferStats(): { capturedBars: number; nonEmptyBars: number; totalEvents: number } {
     ...
     */
    private _generatePatternCode(takeBars: number = 4, currentBeat?: number): string {
        ...
        for (const [voiceName, events] of this._eventBuffer) {
            ...
            // Create empty grid
            const steps: string[] = Array(totalSteps).fill('.');
     ...
     */
    private async _writeCodeToFile(code: string) {
...
<body>
    ...
    ${renderKeyAssignmentPanel('voices', 'Numpad Recording', 'Press 1-9 to trigger voices', true)}
